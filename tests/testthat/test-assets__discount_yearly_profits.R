...
testthat::test_that("discount_yearly_profits validates inputs", {
  yearly_scenarios <- data.frame(
    asset = "A1",
    company = "C1",
    year = 2025,
  ...
  testthat::expect_no_error(discount_yearly_profits(yearly_scenarios, 0.05))

  # Should error with invalid inputs
  testthat::expect_error(
    discount_yearly_profits(NULL, 0.05),
    regexp = "non-empty data.frame"
  )

  testthat::expect_error(
    discount_yearly_profits(yearly_scenarios, "invalid"),
  ...
  testthat::expect_error(
    discount_yearly_profits(incomplete_data, 0.05),
    regexp = "Missing required columns"
  )
})

testthat::test_that("discount_yearly_profits handles edge cases", {
  ...
  result <- discount_yearly_profits(yearly_scenarios, 0.05)
  testthat::expect_true(result$discounted_net_profit < 0)
  testthat::expect_equal(result$discounted_net_profit, -50) # No discounting for base year

  # Test with zero profits
  yearly_scenarios_zero <- data.frame(
    asset = "A1",
    company = "C1",
    year = 2025,
    scenario = "baseline",
  ...
  result_zero <- discount_yearly_profits(yearly_scenarios_zero, 0.05)
  testthat::expect_equal(result_zero$discounted_net_profit, 0)
})
