...
test_that("extract_csv_statistics matches output format of other extractors", {
  assets_df <- data.frame(
    asset = "A1",
    company = "C1",
    latitude = -32.5,
  ...
  required_cols <- c(
    ...
    "hazard_indicator", "hazard_return_period", "scenario_code",
    "scenario_name", "source", "hazard_intensity", "matching_method"
  )

  expect_true(all(required_cols %in% names(result)))

  # Check specific values
  expect_equal(result$source, "csv")
  expect_equal(result$matching_method, "coordinates")
  expect_equal(result$hazard_type, "TestHazard")
  expect_equal(result$hazard_indicator, "HI")
  expect_equal(result$hazard_return_period, 5)
})

test_that("extract_csv_statistics handles edge cases", {
  # Test with single CSV point
  assets_df <- data.frame(
    asset = c("A1", "A2"),
    company = c("C1", "C1"),
    latitude = c(-32.0, -33.0),
    longitude = c(-53.0, -54.0),
    municipality = c(NA, NA),
    province = c(NA, NA),
    asset_category = c("building", "building"),
    size_in_m2 = c(1000, 1000),
  ...
  # Both assets should get the same value (only point available)
  expect_equal(nrow(result), 2)
  expect_equal(result$hazard_intensity[1], 75)
  expect_equal(result$hazard_intensity[2], 75)
})
