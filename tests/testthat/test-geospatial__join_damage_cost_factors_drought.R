...
# testthat::test_that("join_drought_damage_factors: intensity > -1 should have zero damage", {
#   base_dir <- get_test_data_dir()

#   # Test with intensity > -1 (no drought condition)
#   assets_long <- data.frame(
#     asset = c("A1", "A2", "A3"),
#     company = c("C1", "C2", "C3"),
#     latitude = c(-10, -15, -20),
...
#     asset_subtype = c("Soybean", "Corn", "Sugarcane"),
#     size_in_m2 = c(10000, 8000, 12000),
#     share_of_economic_activity = c(0.5, 0.3, 0.6),
...
#   testthat::expect_true(all(is.na(out$growing_season)))
#   testthat::expect_true(all(is.na(out$off_window)))
# })

# testthat::test_that("join_drought_damage_factors: extreme intensity values (boundary testing)", {
#   base_dir <- get_test_data_dir()
...
#     hazard_type = c("Drought", "Drought", "Drought"),
#     hazard_indicator = c("SPI3", "SPI3", "SPI3"),
#     hazard_intensity = c(-5.0, -10.0, -1.01), # Extreme low, very extreme, just below threshold
#     scenario_name = c("present", "present", "present"),
#     event_id = c("event_1", "event_1", "event_1"),
#     event_year = c(2030, 2030, 2030),
...
#   testthat::expect_equal(nrow(out), 3)
#   # All should have damage factors since intensity <= -1
