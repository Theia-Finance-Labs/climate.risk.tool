...
events <- data.frame(
  hazard_type = c("flood", "CDD"),
  hazard_name = c(
    "flood__rcp85_h100glob", # TIF flood hazard (no "global_" prefix)
    "Drought__SPI6__GWL=present__RP=25__ensemble=mean" # NC drought hazard (CDD indicator)
  ),
  event_year = c(2030, 2040), # Different years for different events
  chronic = c(FALSE, TRUE) # Mix of acute and chronic events
)
```
