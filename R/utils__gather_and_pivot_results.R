...
gather_and_pivot_results <- function(assets_discounted, companies_expected_loss) {
  ...
  if (length(missing_asset_cols) > 0) {
    stop(paste(
      "Missing required columns in assets_discounted:",
  ...
  if (length(missing_company_cols) > 0) {
    stop(paste(
      "Missing required columns in companies_expected_loss:",
  ...
  asset_id_cols <- setdiff(
    names(assets_discounted),
