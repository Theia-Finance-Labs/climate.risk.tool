...
#' multiple events with type, scenario, event year, and chronic toggle.
...
#'
#' @param hazards_inventory reactive data.frame with columns: key, hazard_type, scenario
#' @return reactive data.frame of configured events with columns: event_id, hazard_key, event_year, chronic
#' @export
mod_hazards_events_server <- function(id, hazards_inventory) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$add_event, {
      k <- counter()
      # Only append if both hazard and scenario are chosen
      haz <- input[[paste0("hazard_", k)]]
      scn <- input[[paste0("scenario_", k)]]
      if (is.null(haz) || is.null(scn) || length(scn) == 0) {
    ...
    output$events_ui <- shiny::renderUI({
      ...
      if (!inherits(inv, "try-error") && is.data.frame(inv) && nrow(inv) > 0) {
        ...
        if (length(hazard_choices) > 0) {
          scenario_choices <- unique(inv$scenario[inv$hazard_type == hazard_choices[[1]]])
        }
      }
      ...
      shiny::wellPanel(
        ...
        shiny::uiOutput(ns(paste0("scenario_ui_", k))),
        shiny::checkboxInput(ns(paste0("chronic_", k)), label = "Chronic hazard (every year)", value = FALSE),
        shiny::uiOutput(ns(paste0("year_ui_", k)))
      )
    })

    # Scenario UI reacts to hazard selection for current event
    shiny::observe({
      k <- counter()
      if (k == 0) return()
      
      output[[paste0("scenario_ui_", k)]] <- shiny::renderUI({
        inv <- try(hazards_inventory(), silent = TRUE)
        hazard_val <- input[[paste0("hazard_", k)]]
        scen <- character(0)
        if (!inherits(inv, "try-error") && is.data.frame(inv) && !is.null(hazard_val)) {
          scen <- unique(inv$scenario[inv$hazard_type == hazard_val])
        }
        shiny::selectInput(ns(paste0("scenario_", k)), "Scenario", choices = scen, selected = if (length(scen) > 0) scen[[1]] else NULL)
      })
