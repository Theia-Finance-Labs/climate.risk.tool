...
#'
#' @title Read precomputed hazard statistics for states and municipalities
#' @description Reads precomputed hazard statistics from CSV file containing hazard data
#'   aggregated at ADM1 (state) and ADM2 (municipality) levels. Used to look up hazard
#'   values for assets without coordinates but with state or municipality information.
...
#'   p2_5, p5, p95, p97_5. adm_level is "ADM1" for states or "ADM2" for municipalities.
...
#' # Look up Amazonas state flood hazard
...
read_precomputed_hazards <- function(base_dir) {
  ...
  message("  ADM1 (state) records: ", sum(precomputed_df$adm_level == "ADM1"))
...
#'   - state: Named character vector mapping normalized state names to original names
...
#' # Access original name: name_mapping$state["Sao Paulo"] returns "São Paulo"
#' }
#' @export
load_region_name_mapping <- function(base_dir) {
  # Initialize result list
  mapping <- list(state = character(0), municipality = character(0))

  # Load state (ADM1) names
  state_path <- file.path(base_dir, "areas", "state", "geoBoundaries-BRA-ADM1_simplified.geojson")
  if (file.exists(state_path)) {
    states_sf <- sf::st_read(state_path, quiet = TRUE)

    if ("shapeName" %in% names(states_sf)) {
      # Get original names
      original_names <- as.character(states_sf$shapeName)
      ...
      mapping$state <- original_names
      names(mapping$state) <- normalized_names
    }
  }
  ...
  message(
    "[load_region_name_mapping] Loaded ", length(mapping$state), " state names and ",
