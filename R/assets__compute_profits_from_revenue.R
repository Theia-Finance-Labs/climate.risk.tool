...
compute_profits_from_revenue <- function(
  ...
  # Validate inputs
  if (!"company" %in% names(yearly_revenue_df)) {
    stop("yearly_revenue_df must contain a 'company' column")
  }
  if (!"company" %in% names(companies)) {
    stop("companies must contain a 'company' column")
  }
  if (!"net_profit_margin" %in% names(companies)) {
    stop("companies must contain a 'net_profit_margin' column")
  }
