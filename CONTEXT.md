...
- Batched terra-based raster extraction for NetCDF and TIF hazards with persistent progress logging
- Used for assets WITH coordinates
- Returns `matching_method = "coordinates"`
- Includes `season` and `ensemble` columns for traceability
- Reuses cached asset geometries per raster CRS and dispatches to separate point vs polygon extraction helpers
...
### Bug Fixes
- **Auto-repaired invalid spatial separation geometries and surfaced the warning in analysis status** (2026-04-21): `load_spatial_separation_data()` now validates ADM and hydro region layers on load, repairing invalid polygons with `sf::st_make_valid()` using the GEOS path (temporarily disabling `s2` for the repair step). When repairs happen, the code emits explicit `[spatial_separation] ...` warnings so the data issue is not hidden, and the app appends those warnings to the final analysis status text after a successful run. Added regression coverage in `test-utils__spatial_separation.R`.
- **Preloaded spatial separation data once at app load instead of lazily reloading it in the event UI** (2026-04-21): `app_server()` now loads and caches `spatial_separation_data` alongside the other static files, including any geometry-repair warnings. `mod_control_server()` and `mod_hazards_events_server()` can now consume a preloaded spatial-data reactive, so region dropdowns reuse the cached repaired layers instead of re-reading shapefiles on demand. The final analysis status still includes any cached spatial-separation warnings. Added regression coverage in `test-mod_hazards_events.R`.
- **Refactored spatial extraction around vectorized geometries and CRS-aware helper pipelines** (2026-04-21): `create_asset_geometries()` now builds all point buffers in a single vectorized `sf` pipeline instead of row-by-row mapping. `extract_spatial_statistics()` now resolves hazard execution plans up front, groups hazards by extraction mode and raster CRS, caches transformed geometry views, and routes work through dedicated point/polygon extraction helpers. Spatial batching is now engine-side and adaptive per hazard raster: point extraction uses larger batches, while polygon extraction derives a larger default batch size from raster cell count and only recursively splits dispersed assets when the crop footprint would exceed a cell threshold. Benchmarked on `workspace/demo_inputs_spatial_separation`, the `compute_risk()` demo scenario with drought + flood completed with the flood extraction finishing in about 2 seconds and the whole extraction phase in about 5.5 seconds. Added regression coverage in `test-geospatial__create_asset_geometries.R` and expanded the progress-log assertions in `test-geospatial__extract_hazard_statistics.R`.
- **Added persistent hazard extraction progress + smaller spatial batches** (2026-04-21): Updated `extract_spatial_statistics()` to process all raster hazards (both NetCDF-derived rasters and TIF rasters) in small asset batches, logging per-batch elapsed time and ETA so long-running extractions leave readable progress history in the console. Each polygon batch now crops the raster to the batch extent before calling `terra::extract()`, which reduces work when assets are geographically dispersed. Added regression coverage in `tests/testthat/test-geospatial__extract_hazard_statistics.R` for the new batch progress messages.
